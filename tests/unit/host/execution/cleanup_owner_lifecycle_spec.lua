...
describe('CleanupOwnerLifecycle', function()
    ...
    it('accepts exactly one behavior status for each entered attempt', function()
        local owner_lifecycle = lifecycle()
        owner_lifecycle:suite_entry(suite_identity())
        enter_test(owner_lifecycle)
        owner_lifecycle:test_exit('test teardown', false)
        owner_lifecycle:test_finished('pending')
        assert_error(function()
            owner_lifecycle:test_finished('success')
        end, 'no completed test attempt')
    end)
end)
