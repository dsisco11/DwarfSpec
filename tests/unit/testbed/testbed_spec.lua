...
describe('dwarfspec.testbed entry point', function()
    ...
    it('loads when DFHack globals are unavailable', function()
        ...
        assert.is_nil(testbed.ensure_open)
        assert.is_nil(testbed.reload)
        assert.is_nil(testbed.inspect)
    end)
    ...
    it('constructs a provider-only bed through the public constructor', function()
        ...
        assert.is_nil(rawget(bed, 'guard'))
        assert.is_nil(rawget(bed, 'package_state'))
        assert.is_nil(rawget(bed, 'script_loader'))
        assert.is_table(rawget(bed, '_guard'))
        assert.is_table(rawget(bed, '_package_state'))
        assert.is_table(rawget(bed, '_script_loader'))
        assert.is_true(TestBed.is_instance(bed))
