...
- All game text uses `font.baseGame` at the established 12 px game-text
  size.
- Every foreground, background, and border color is linked to the existing DF
  palette. The specimens do not introduce literal design colors.
- Non-printable and extended visual symbols use the existing CP437 atlas
  assets. Printable ASCII emitted literally by the installed widget remains
  `font.baseGame` text because the supplied atlas-asset catalog omits printable
  ASCII cells; no Unicode or newly drawn substitute is permitted.
...
- Every widget page and the coverage page has one appropriately named
  top-level board and Canvas Background `#1b1b1b`.
- All text on those pages uses `font.baseGame`, VT323, and the single 12 px
  game-text size.
- All solid colors and strokes are linked to the supplied DF palette. No
  numeric stand-in text remains for CP437 glyphs.
...
- ConfigureButton uses CP437 15. Slider and RangeSlider use the locally
  rendered CP437 9, 181, 198, 205, and 216 assets. Panel, Divider, and
  DimensionsTooltip use the supplied frame/junction assets.
